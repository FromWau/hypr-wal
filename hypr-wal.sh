...
updateKitty() {
    ...
    [[ $(rg "^include wal-theme.conf\$" "$HOME"/.config/kitty/kitty.conf) == "" ]] &&
        echo "Pls run following line to include wal-theme to your $HOME/.cache/wal/colors-kitty.conf:" &&
        echo "echo 'include wal-theme.conf' >> $HOME/.cache/wal/colors-kitty.conf"
}
